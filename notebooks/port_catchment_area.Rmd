...
# commented out image sizing moifications
...
```{r import fishing data for Cotonou Port Catchment Map, echo=F, include=F, warning=F}

fishing_ben <- fishwatchr::gfw_query("
    SELECT 
      * 
    FROM `world-fishing-827.PortsProgramme.BEN_events_2021-23` 
    WHERE event_type = 'Fishing'",
 run_query = TRUE,
 con = con
)$data
...
# produce port catchment map
(cotonou_fishing_catchment <- fishing_ben %>%
  sf::st_as_sf(., coords = c('lon_mean','lat_mean'), crs = 4326) %>%
  fishwatchr::recenter_sf(.) %>%
ggplot() +
  geom_sf(alpha = 1, 
  ...
  geom_sf(data = fao_major,
    ...
    coord_sf(xlim = c(bbox$box_out[['xmin']], bbox$box_out[['xmax']]),
             ylim = c(bbox$box_out[['ymin']], bbox$box_out[['ymax']]),
             crs = bbox$out_crs) +
    labs(title = glue::glue("Fishing before entering Cotonou, Benin"))
)

# save output as pdf
ggsave(cotonou_fishing_catchment, 
       filename = here::here('outputs','figures','catchment_maps',paste('cotonou_benin_catchment_map_v',Sys.Date(),'.pdf', sep='')),
       device = cairo_pdf,
       width = 150,
       height = 150,
       units = 'mm',
...
# save output as png
ggsave(cotonou_fishing_catchment, 
       filename = here::here('outputs','figures','catchment_maps',paste('cotonou_benin_catchment_map_v',Sys.Date(),'.png', sep='')),
       device = 'png',
       width = 150,
       height = 150,
       units = 'mm',
...
```{r gear heatmap benin, warning=F, echo=F}

# set plotting res for heatmaps
res <- 0.1

# create binned values
fishing_ben$lat_bin <- round(fishing_ben$lat_mean/res)*res
fishing_ben$lon_bin <- round(fishing_ben$lon_mean/res)*res

# look at top gears 
table(fishing_ben$geartype_best)
table(fishing_ben$shipname)
# only three vessel make a combined heatmap

# check limits
test_lim <- fishing_ben %>%
    group_by(lat_bin, lon_bin) %>%
    summarize(fishing_hours = sum(event_duration_hrs, na.rm = TRUE), .groups = 'drop')
quantile(test_lim$fishing_hours)
...
ggplot() +
  ...
  geom_sf(data = fao_major,
    ...
    coord_sf(xlim = c(bbox$box_out[['xmin']], bbox$box_out[['xmax']]),
             ylim = c(bbox$box_out[['ymin']], bbox$box_out[['ymax']]),
             crs = bbox$out_crs) +
    labs(title = glue::glue("Fishing"), fill = 'Fishing Hours') 


ggsave(fishing_heatmap, 
       filename = here::here('outputs','figures','catchment_maps',paste('cotonou_gear_catchment_heatmap_v',Sys.Date(),'.png', sep='')),
       device = 'png',
       width = 150,
       height = 150,
       units = 'mm',
...
```{r import fishing data for Lome Port Catchment Map, echo=F, include=F, warning=F}

fishing_tog <- fishwatchr::gfw_query("
    SELECT 
      * 
    FROM `world-fishing-827.PortsProgramme.TGO_events_2021-23` 
    WHERE event_type = 'Fishing'",
 run_query = TRUE,
 con = con
)$data
...
```{r Port Catchment Map Lome, echo=F, warning=F}
# set bounding box limits 
bbox <- transform_box(
  xlim = c(-4, 4),
  ylim = c(2, 7),
...
# produce port catchment map
(lome_fishing_catchment <- fishing_tog %>%
  sf::st_as_sf(., coords = c('lon_mean','lat_mean'), crs = 4326) %>%
  fishwatchr::recenter_sf(.) %>%
ggplot() +
  geom_sf(alpha = 1, 
  ...
  geom_sf(data = fao_major,
    ...
    coord_sf(xlim = c(bbox$box_out[['xmin']], bbox$box_out[['xmax']]),
             ylim = c(bbox$box_out[['ymin']], bbox$box_out[['ymax']]),
             crs = bbox$out_crs) +
    labs(title = glue::glue("Fishing before entering Lome, Togo"))
)

# save output as pdf
ggsave(lome_fishing_catchment, 
       filename = here::here('outputs','figures','catchment_maps',paste('lome_togo_catchment_map_v',Sys.Date(),'.pdf', sep='')),
       device = cairo_pdf,
       width = 200,
       height = 150,
       units = 'mm',
...
# save output as png
ggsave(lome_fishing_catchment, 
       filename = here::here('outputs','figures','catchment_maps',paste('lome_togo_catchment_map_v',Sys.Date(),'.png', sep='')),
       device = 'png',
       width = 200,
       height = 150,
       units = 'mm',
...
```{r gear heatmap togo, warning=F, echo=F}

# set plotting res for heatmaps
res <- 0.1

# create binned values
fishing_tog$lat_bin <- round(fishing_tog$lat_mean/res)*res
fishing_tog$lon_bin <- round(fishing_tog$lon_mean/res)*res

# look at top gears 
table(fishing_tog$geartype_best)
table(fishing_tog$shipname)
# only eight vessel make a combined heatmap

# check limits
test_lim <- fishing_tog %>%
    group_by(lat_bin, lon_bin) %>%
    summarize(fishing_hours = sum(event_duration_hrs, na.rm = TRUE), .groups = 'drop')
quantile(test_lim$fishing_hours)
...
ggsave(fishing_heatmap, 
       filename = here::here('outputs','figures','catchment_maps',paste('lome_gear_catchment_heatmap_v',Sys.Date(),'.png', sep='')),
       device = 'png',
       width = 200,
       height = 150,
       units = 'mm',
       dpi = 600)
```
```{r import voyage ais data for Lome port profile voyages, echo=F, include=F, warning=F}

start_date <- "'2021-01-01 00:00:00 UTC'"
end_date <- "'2024-01-01 00:00:00 UTC'"
voyages_table <- "`world-fishing-827.PortsProgramme.TGO_voyages_2021-23`"

voyages_ais_q <- readr::read_file(here::here("queries","port_catchment", "voyage_ais_data.sql"))
...
bounding_atlan <- transform_box(xlim = c(-20, 15),
                          ylim = c(-5, 35),
                          output_crs = best_proj)
...
tog_carrier_map <- 
  ...
  geom_sf(data = voyage_ais_line, aes(colour=colour), alpha=0.2) +
...
tog_carrier_map_glob <- fishwatchr::add_little_globe(tog_carrier_map,
...
# save object 
ggsave(tog_carrier_map_glob, 
       filename = here::here('outputs','figures','catchment_maps',
                             paste('lome_carrier_lines_highres_v',Sys.Date(),'.png', sep='')), 
       width = 8, 
       height = 10)

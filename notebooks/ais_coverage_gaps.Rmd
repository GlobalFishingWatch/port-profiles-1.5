...
res <- 0.5
west_africa_full$lat_bin2 <- round(west_africa_full$lat_bin / res) * res 
west_africa_full$lon_bin2 <- round(west_africa_full$lon_bin / res) * res

waf <- west_africa_full %>% 
...
atlantic_map <- data.frame(sat_perf) %>%
  ...
  geom_raster(aes(x= lon_bin2, y=lat_bin2, fill = sat_prop)) +
...
ggsave(here::here("outputs", "figures", "senegal_sat_ais_cov_0.5.png"), width = 10, height = 12)
...
ggsave(here::here("outputs", "figures", "senegal_sat_ais_class_A_cov_v0.5.png"), width = 10, height = 12)
...
ggsave(here::here("outputs", "figures", "senegal_sat_ais_class_B_cov_v0.5.png"), width = 10, height = 12)
...
ggsave(here::here("outputs", "figures", "senegal_terr_ais_cov0.5.png"), width = 10, height = 12)

```
# AIS coverage Alternate

```{r download proto messages tyler, echo=F, warning=F}
q_coverage_v3 <- readr::read_file(file = here::here("queries", "ais_coverage", "AIS_coverage_Tyler.sql"))

WA_class_a <- fishwatchr::gfw_query(query = glue::glue(q_coverage_v3,
                  start_date = "'2023-01-01'",
                  end_date = "'2023-07-01'", 
                  min_lat = "10", 
                  max_lat = "17", 
...
```{r Test different coverage metrics class A, echo=F, warning=F}

# res <- 0.5
# WA_class_a$lat_bin2 <- round(WA_class_a$lat_bin / res) * res 
# WA_class_a$lon_bin2 <- round(WA_class_a$lon_bin / res) * res
# 
# waf <- filter(WA_class_a, type=='A') %>% 
#   group_by(lat_bin2, lon_bin2) %>% 
#     summarise(sat_pos = sum(sat_pos),
#               sat_pos_ttl  = sum(sat_pos_ttl),
#               segments = sum(segments))
...
class_a <- filter(WA_class_a, type=='A')
...
atlantic_map <- class_a %>%
  recenter_raster(raster_df = .,
                  res = 1,
                  x_lab = 'lon_bin',
                  y_lab = 'lat_bin',
                  fill_lab = 'satellite_reception',
                  center = new_center) %>% 
  ggplot()+
  geom_gfw_outline(center = new_center, theme = "dark") +
  geom_raster(aes(x= lon_bin, y=lat_bin, fill = satellite_reception)) +
    geom_gfw_eez(theme = 'light', alpha = 0.8, center = new_center) +
    geom_gfw_land(theme = 'dark', center = new_center) +
    scale_fill_gradientn(colors = gfw_palette('map_effort_dark'),
                       limits = c(0,1),
...
ggsave(here::here("outputs", "figures", "senegal_classA_ais_cov_v2.png"), width = 10, height = 12)

```

```{r Test different coverage metrics class B, echo=F, warning=F}

class_a <- filter(WA_class_a, type=='B')
...
# set plot limits for Pacific 
bounding_atlantic <- transform_box(xlim = c(-20.5, -15),
                          ylim = c(10, 16.5),
                          output_crs = best_proj)
...
ggsave(here::here("outputs", "figures", "senegal_classB_ais_cov_v2.png"), width = 10, height = 12)

```

```{r Test different coverage metrics Terrestrial, echo=F, warning=F}
...
atlantic_map <- class_a %>%
  recenter_raster(raster_df = .,
                  ...
                  x_lab = 'lon_bin',
                  y_lab = 'lat_bin',
                  fill_lab = 'terrestrial_reception',
                  center = new_center) %>% 
  ggplot()+
  geom_gfw_outline(center = new_center, theme = "dark") +
  geom_raster(aes(x= lon_bin, y=lat_bin, fill = terrestrial_reception)) +
    geom_gfw_eez(theme = 'light', alpha = 0.8, center = new_center) +
    geom_gfw_land(theme = 'dark', center = new_center) +
    scale_fill_gradientn(colors = gfw_palette('map_effort_dark'),
                       limits = c(0,1),
    ...
    coord_sf(xlim = c(bounding_atlantic$box_out[['xmin']], bounding_atlantic$box_out[['xmax']]),
           ylim = c(bounding_atlantic$box_out[['ymin']], bounding_atlantic$box_out[['ymax']]),
           crs = bounding_atlantic$out_crs) +
      labs(fill = 'Terrestrial coverage') +
      theme_gfw_map(theme = 'light') 
...
ggsave(here::here("outputs", "figures", "senegal_terrestrial_ais_cov_v2.png"), width = 10, height = 12)
...
class_a <- filter(measured_ais, ais_class == 'A')
...
ggsave(here::here("outputs", "figures", "senegal_A_measure_cov.png"), width = 10, height = 12)

```

```{r Test measured AIS class B, echo=F, warning=F}

class_a <- filter(measured_ais, ais_class == 'B')
...
ggsave(here::here("outputs", "figures", "senegal_B_measure_cov.png"), width = 10, height = 12)

...
CREATE or replace TABLE `world-fishing-827.PortsProgramme.MDG_masterlist_2022-24` AS
...
voyages AS (
  ...
  FROM `world-fishing-827.PortsProgramme.MDG_voyages_2022-24` AS a
  LEFT JOIN (
    SELECT
      trip_id,
      -- total_voyage_h,
      -- total_ais_h,
      percent_ais_voyage
    FROM `world-fishing-827.PortsProgramme.MDG_aiscoverage_2022-24`) AS b
  USING (trip_id)
  ),
...
add_port_events AS(
  ...
  LEFT JOIN (
    ...
    FROM `global-fishing-watch.pipe_ais_v3_published.port_visits`
...
add_anchorages AS(
  ...
  FROM (
    ...
    -- a.trip_end_anchorage_id,
      ...
      FROM `world-fishing-827.PortsProgramme.MDG_anchorages_reviewed`) as b
...
events AS (
  ...
  FROM `world-fishing-827.PortsProgramme.MDG_events_2022_24` AS a
  LEFT JOIN (
    SELECT
      trip_id,
      vessel_id,
...
id_spoofers AS(
  SELECT ssvid
  FROM `global-fishing-watch.pipe_ais_v3_published.vi_ssvid_byyear_v20240701`
  WHERE

...
--------------------------------------------------
-- pull events for missing vessels - note no voyage associated, need to manual enter ssvid/id and dates
--------------------------------------------------
...
voyages AS (
  SELECT
    ...
    trip_start,
    trip_end,
  FROM `world-fishing-827.PortsProgramme.MDG_voyages_2022-24`
  ),
...
encounters AS (
  SELECT
    ...
    CAST(NULL AS string) AS vessel_flag_best,
    CAST(NULL AS string) AS callsign,
    CAST(NULL AS string) AS imo,
    CAST(NULL AS string) AS vessel_class_best,
    CAST(NULL AS string) AS geartype_best,
    CAST(NULL AS string) AS vessel_class_initial,
    CAST(NULL AS string) AS class_confidence_initial,
  ...
  FROM(
    SELECT
      ...
      FROM `global-fishing-watch.pipe_ais_v3_published.product_events_encounter`
      ) enc
    -- INNER JOIN voyages
    --   ON
    WHERE
    enc.event_start BETWEEN TIMESTAMP('2024-03-11 00:00:00 UTC') AND TIMESTAMP('2024-04-05 00:00:00 UTC')
    AND vessel_id IN (select vessel_id FROM `global-fishing-watch.pipe_ais_v3_published.product_vessel_info_summary` WHERE ssvid IN ("664271000") AND YEAR = 2024)
    -- WHERE
    --   product_shiptype != 'gear' AND enc_product_shiptype != 'gear'
...
loitering AS(
  ...
  FROM(
    SELECT
      ...
      start_distance_from_port_km,
      end_distance_from_port_km
      FROM `global-fishing-watch.pipe_ais_v3_published.product_events_loitering`
      WHERE
        seg_id IN (
        SELECT
          seg_id
        ...
        WHERE
          ...
          AND SAFE_CAST(JSON_QUERY(event_info,"$.avg_distance_from_shore_km") AS FLOAT64) > 37.04
    ...
    WHERE
    loit.event_start BETWEEN TIMESTAMP('2024-03-11 00:00:00 UTC') AND TIMESTAMP('2024-04-05 00:00:00 UTC')
AND vessel_id IN (select vessel_id FROM `global-fishing-watch.pipe_ais_v3_published.product_vessel_info_summary` WHERE ssvid IN ("664271000") AND YEAR = 2024)
...
fishing AS(
  ...
  FROM(
    SELECT
      ...
      start_distance_from_port_km,
      end_distance_from_port_km
      FROM `global-fishing-watch.pipe_ais_v3_published.product_events_fishing`
      ) fish
     WHERE
    fish.event_start BETWEEN TIMESTAMP('2024-03-11 00:00:00 UTC') AND TIMESTAMP('2024-04-05 00:00:00 UTC')
    AND vessel_id IN (select vessel_id FROM `global-fishing-watch.pipe_ais_v3_published.product_vessel_info_summary` WHERE ssvid IN ("664271000") AND YEAR = 2024) ),
...
gaps AS(
  ...
  FROM(
    SELECT
      ...
      FROM `global-fishing-watch.pipe_ais_v3_published.product_events_ais_gaps`
      ) gaps
        WHERE
    gaps.event_start BETWEEN TIMESTAMP('2024-03-11 00:00:00 UTC') AND TIMESTAMP('2024-04-05 00:00:00 UTC')
    AND ssvid IN (select ssvid FROM `global-fishing-watch.pipe_ais_v3_published.product_vessel_info_summary` WHERE ssvid IN ("664271000") AND YEAR = 2024)
    ),
...
encounter_v_info AS(
  ...
  FROM(
    ...
    LEFT JOIN (
      ...
      FROM
        `global-fishing-watch.pipe_ais_v3_published.product_vessel_info_summary`) vi
      ON

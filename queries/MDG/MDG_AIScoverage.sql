...
CREATE TEMP FUNCTION start_timestamp() AS (TIMESTAMP('2022-01-01 00:00:00 UTC'));
--
# voyages will be truncated to this end timestamp, if needed
CREATE TEMP FUNCTION end_timestamp() AS (TIMESTAMP('2024-12-31 23:59:59 UTC'));
--
------------------------------------------------------------

CREATE TABLE `world-fishing-827.PortsProgramme.MDG_aiscoverage_2022-24` AS
...
############################################################
------------------------------------------------------------
-- Raw data from the voyage table
------------------------------------------------------------
  MDG_trip_ids AS (
    SELECT
      trip_id,
...
messages AS (
  ...
  AND a.timestamp BETWEEN start_timestamp() AND end_timestamp()
  AND a.vessel_id IN (SELECT vessel_id FROM MDG_trip_ids)
),

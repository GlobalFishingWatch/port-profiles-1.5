...
--------------------------------------------------
-- makes table of only port events to 4 select ports highlighted by Jay, for
-- all vessels NOT included in original NGA dataset
--------------------------------------------------

-- Name desired output table
CREATE or replace TABLE `world-fishing-827.PortsProgramme.NGA_masterlist_2021-23_missinganchorages` AS
...
voyages AS (
  SELECT
    ...
    class_confidence_initial AS class_confidence,
    trip_start,
    start_port_iso3,
    start_port_label,
    trip_end,
    ...
    -- a.tmt_notes,
    trip_duration_days AS total_voyage_days,
      -- b.total_voyage_h,
      -- b.total_ais_h,
      -- percent_ais_voyage,
    start_portvisit_timestamp AS event_start,
    end_portvisit_timestamp AS event_end,
    port_event_duration_days AS event_duration_d,
      CAST(NULL AS numeric) AS lat_mean,
      CAST(NULL AS numeric) AS lon_mean,
    CAST(start_latitude AS numeric) AS lat_start,
    CAST(start_longitude AS numeric) AS lon_start,
  ...
  FROM `world-fishing-827.scratch_joef.testing_missed_anchorages_NGA`--AS a
  -- LEFT JOIN (
  --   SELECT
  --     trip_id,
  --     -- total_voyage_h,
...
add_anchorages AS(
  ...
  FROM (
    ...
    -- a.trip_end_anchorage_id,
      ...
      FROM `world-fishing-827.PortsProgramme.NGA_all_anchorages_withTMT`) as b
      -- USING (s2id) )
...
events AS (
  ...
  FROM `world-fishing-827.scratch_joef.testing_missed_anchorages_NGA_events` AS a
  LEFT JOIN (
    SELECT
      trip_id,
      vessel_id,
...
trips_to_anchorages AS(
  SELECT DISTINCT
    trip_id,
    anchorage_id
  FROM all_events
  WHERE anchorage_id IN("103b8a2f", "103b8a31", "103b8ae3", "103b8afd")
)
...
SELECT
  ...
  possible_spoofing
FROM all_events
WHERE trip_id IN (select trip_id FROM trips_to_anchorages)
AND event_type IN ("Port_Visit")

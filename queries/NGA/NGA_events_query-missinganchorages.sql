...
-- this version is for missing Lagos anchorage exploration
--------------------------------------------------
...
CREATE or replace TABLE `world-fishing-827.scratch_joef.testing_missed_anchorages_NGA_events` AS
...
voyages AS (
  SELECT
    ...
    trip_start,
    trip_end,
  FROM `scratch_joef.testing_missed_anchorages_NGA`
  ),
...
loitering AS(
  ...
  FROM(
    SELECT
      ...
      start_distance_from_port_km,
      end_distance_from_port_km
      FROM `pipe_ais_v3_published.product_events_loitering`
      WHERE
        seg_id IN (
        SELECT
          seg_id
...
fishing AS(
  ...
  FROM(
    SELECT
      ...
      start_distance_from_port_km,
      end_distance_from_port_km
      FROM `pipe_ais_v3_published.product_events_fishing`
      ) fish
    INNER JOIN voyages
      ON
...
gaps AS(
  ...
  FROM(
    SELECT
      ...
      FROM `pipe_ais_v3_published.product_events_ais_gaps`
      ) gaps
    INNER JOIN voyages
      ON

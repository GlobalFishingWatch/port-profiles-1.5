...
CREATE TABLE `world-fishing-827.PortsProgramme.LBR_voyages_2021-23` AS
...
----------------------------------------------------------
-- add in vessels not present on vessel list (ie TMT flagged) (2020 for the hau yuan fleet voyages arriving in 2020)
----------------------------------------------------------
 ...
 additional_vessels AS(
   SELECT DISTINCT
    ...
    WHERE
     ...
     AND ssvid IN ("311040500", "309361000", "636019354", "412483273", "412483274", "412483275", "412483279", "412479022") -- add additional vessels here
    ),
  ...
  fishing_voyages AS (
    ...
    FROM (
      ...
      WHERE
        ...
        ## manually adding in Hau Yuan Yu fleet voyages arriving in Monrovia (in 2020)
        OR ( trip_start <= end_date() AND
        trip_id IN (
            "412479022-708d6a479-95e6-7d80-7101-1b0239f8a63b-01726628bf70",
            "412483273-2dfc90329-9cc4-041d-0dae-69e60ef22711-01726621fcf8",
            "412483274-2186e505a-a159-78d6-2d2c-f111f0e811a1-0172662452a0",
            "412483275-a26755864-4606-3df7-e1c6-375095ac2cba-0172662519d8",
            "412483279-1bcb955fd-d7b0-93fb-c1de-71cbacedfa07-01726621f528"
        ))
        )
...
----------------------------------------------------------
  nn_bunkers_high_confidence AS (
    ...
    WHERE
      ...
      AND ssvid IN (
        SELECT ssvid
        FROM `pipe_ais_v3_published.identity_core_v20240701`
...
--------------------------------------
  ...
  # filter for ports of interest -- using anchorages list for Monrovia in this case
      OR ssvid IN ("311040500", "309361000", "636019354", "412483273", "412483274", "412483275", "412483279", "412479022") -- adding in TMT missing vessels

...
-- CREATE TABLE `world-fishing-827.scratch_joef.LBR_NGA_aiscoverage_2021-23_pipe3` AS
...
--
------------------------------------------------------------
-- Raw data from the voyage table, doing LBR and NGA together to save costs on messages table
------------------------------------------------------------
  LBR_trip_ids AS (
    SELECT
      trip_id,
...
messages AS (
  ...
  WHERE vessel_id IN (SELECT vessel_id FROM all_trips)
),

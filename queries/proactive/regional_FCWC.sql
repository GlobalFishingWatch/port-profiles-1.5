--------------------------------------
  -- Query to quantify foreign fishing, bunker, and carrier vessel visits and unique vessels .
  --
...
CREATE TEMP FUNCTION  end_date() AS (TIMESTAMP('2025-12-31 23:59:59 UTC'));
...
CREATE TEMP FUNCTION port_iso() AS (["BEN", "GHA", "TGO", "CIV", "NGA", "LBR"]);

## save table if needed:
-- CREATE TABLE `world-fishing-827.PortsProgramme.MDG_voyages_2022-24` AS
...
--    AND ssvid IN ("224782000", "245340000", "412331041", "412331052", "461000126", "461000227", "461000228", "461000229", "636023858", "664271000", "645752000", "677032900") -- add additional vessels here
--   ),
...
----------------------------------------------------------
  nn_bunkers_high_confidence AS (
    ...
    WHERE
      ...
      AND ssvid IN (
        SELECT ssvid
        FROM `global-fishing-watch.pipe_ais_v3_published.identity_core`
...
--------------------------------------
  foreign_voyages AS (
    ...
    WHERE
      ...
      -- AND end_port_iso3 IN (port_iso()) -- for single country
      AND end_port_iso3 IN UNNEST(port_iso()) -- for list/array of countries
      AND vessel_iso3 NOT IN ("UNK")
      ),
...
--------------------------------------
  SELECT
    ...
    count(num_AISdisabling) AS total_AISdisabling

...
num_encounters AS (
  ...
  FROM (
    ...
    FROM `global-fishing-watch.pipe_ais_v4_published.product_events_encounter`) enc
    INNER JOIN (
      SELECT
        vessel_id,
        trip_id,
    ...
    FROM (
      ...
      WHERE
        ...
        SELECT
          seg_id
        FROM
          `global-fishing-watch.pipe_ais_v4_published.segs_activity`
        WHERE
          good_seg IS TRUE
  ...
  num_fishing AS(
    ...
    FROM (
      SELECT
        ...
        ISO_TER1,
      FROM
        `global-fishing-watch.pipe_ais_v4_published.product_events_fishing`
  ...
  num_gaps AS(
    ...
    FROM (
      SELECT
        ...
        event_end
      FROM
        `global-fishing-watch.pipe_ais_v4_published.product_events_proto_ais_disabling`) a
    INNER JOIN (
      SELECT
        vessel_id,
        trip_id,
...
--------------------------------------
  all_voyages AS (
    ...
    FROM
      add_visit_end_2
-- turning the below off to ensure any manual added voyage included
    -- WHERE
  ...
  add_vessel_info AS (
    ...
    JOIN (
      SELECT
        ...
        ssvid,
        year,
        ais_shipname AS shipname,
        ais_callsign AS callsign,
        ais_imo AS imo,
...
--------------------------------------
  clean_info AS(
    SELECT
      ...
      FIRST_VALUE (ais_mmsi_flag) OVER (
        PARTITION BY ssvid, year
        ORDER BY vessel_flag_best DESC) AS ais_mmsi_flag,
...
-- changed to join approach from using ssvid IN
--------------------------------------
port_events AS (
  SELECT DISTINCT
    p.visit_id,
    ...
    p.end_timestamp,
    p.end_anchorage_id
  FROM `global-fishing-watch.pipe_ais_v4_published.port_visits` AS p
  -- This replaces the "ssvid IN (...)" subquery
  INNER JOIN (

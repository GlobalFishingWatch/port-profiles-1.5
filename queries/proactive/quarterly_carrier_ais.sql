...
CREATE TEMP FUNCTION carrier_ids()
...
-------------------------------------------------------------------------
-- take voyages from the quarterly voyage table as the starting point
-------------------------------------------------------------------------
...
-------------------------------------------------------------------------
-- identify all voyages in the focal period for vessels in the quarter
-------------------------------------------------------------------------
...
all_ves_voyages AS (
  ...
  WHERE
    ...
    AND vessel_id IN (SELECT DISTINCT vessel_id FROM q_voyages)
...
-------------------------------------------------------------------------
-- identify voyages that were included in the quarterly summary based on trip_id
-------------------------------------------------------------------------
targets AS (
  SELECT rn FROM ordered_voyages WHERE trip_id IN (SELECT trip_id FROM q_voyages)
),

-------------------------------------------------------------------------
-- find the voyages that preceded the quarterly voyages. Exclude the voyages in the quarter
-------------------------------------------------------------------------
preceding_rows AS (
  SELECT
    o.vessel_id,
  ...
  WHERE
    trip_id NOT IN (SELECT trip_id FROM q_voyages)
  ORDER BY trip_start
),

-------------------------------------------------------------------------
-- merge the quarterly voyages with a table of preceeding voyages
-------------------------------------------------------------------------
voyages_all AS (
    SELECT * FROM q_voyages
  UNION ALL
    SELECT * FROM preceding_rows
),

-------------------------------------------------------------------------
-- Get AIS good segs for the vessels active in the quarter
-------------------------------------------------------------------------

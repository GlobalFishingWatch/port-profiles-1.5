...
WITH
  ...
  high_confidence AS (
     SELECT DISTINCT
       ...
       'high' as confidence,
       year,
       CONCAT(ssvid, '_', year) AS ssvid_year
     FROM
      `world-fishing-827.gfw_research.fishing_vessels_ssvid_v20221001`

     LEFT JOIN (
        SELECT
          ssvid,
          registry_info.best_known_shipname As vessel_name,
          registry_info.best_known_callsign AS IRCS,
          registry_info.best_known_imo As IMO,
        FROM
          `world-fishing-827.gfw_research.vi_ssvid_byyear_v20230901`
      ) ri USING (ssvid)
      WHERE
        year = year()
     ),
  ...
  med_confidence AS (
    ...
    year,
    CONCAT(ssvid, '_', year) AS ssvid_year
    FROM `world-fishing-827.gfw_research.vi_ssvid_byyear_v20230901`
  ...
  low_confidence AS (
    ...
    AND year = year()
   -- AND CONCAT(ssvid, '_', year) NOT IN (SELECT ssvid_year FROM high_confidence)
   -- AND CONCAT(ssvid, '_', year) NOT IN (SELECT ssvid_year FROM med_confidence)
    ),

  ----------------------------------------------------------
  -- combined table info
  ----------------------------------------------------------
  vessel_confidence AS (
    SELECT
      ssvid,
      vessel_name,
      best_flag AS flag,
 ...
 voyages_oi AS (
    ...
    INNER JOIN vessel_confidence fv ON voyages.ssvid = fv.ssvid

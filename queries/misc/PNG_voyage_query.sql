...
CREATE TEMP FUNCTION  start_date() AS (TIMESTAMP('2024-01-01 00:00:00 UTC'));
CREATE TEMP FUNCTION  start_year() AS (2024);
  # voyages will be truncated to this end timestamp, if needed
CREATE TEMP FUNCTION  end_date() AS (TIMESTAMP('2024-12-31 23:59:59 UTC'));
...
CREATE TEMP FUNCTION port_iso() AS (CAST("PNG" AS STRING));
...
-- These are vessels on our best fishing list and that have reliable AIS data
  ...
  high_conf_fishing AS (
     ...
     FROM
      `pipe_ais_v3_published.product_vessel_info_summary_v20250414`
     WHERE
...
-- (e.g. spoofing, offsetting, low activity)
  ...
  med_conf_fishing AS (
    SELECT DISTINCT
      ...
      prod_geartype AS gear_type
    FROM
      `pipe_ais_v3_published.product_vessel_info_summary_v20250414` AS vi_table
    WHERE
    year >= start_year() AND year <= end_year()
...
---------------------------------------------------------------
  reg_carriers AS (
    SELECT DISTINCT -- generics for flag and gear as there are duplicates in v database which will duplicate voyages
      ...
      last_timestamp
    FROM
      `pipe_ais_v3_published.identity_core_v20250301`
    WHERE
...
---------------------------------------------------------------
  nn_carriers_med_confidence AS (
    ...
    FROM
      `pipe_ais_v3_published.vi_ssvid_byyear_v20250301`
    WHERE
...
----------------------------------------------------------
  nn_bunkers_high_confidence AS (
    ...
    WHERE
      ...
      AND ssvid IN (
        SELECT ssvid
        FROM `pipe_ais_v3_published.identity_core_v20250301`
...
-- pull_by_anchorage AS(
  SELECT
    ...
    IF(
      end_port_iso3 != vessel_flag_best,
      TRUE, FALSE) AS is_foreign_visit,
    vessel_id,
    ...
    num_AISdisabling
  FROM clean_info
  WHERE end_port_iso3 IN (port_iso())

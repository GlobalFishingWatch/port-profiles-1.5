...
CREATE TEMP FUNCTION port_iso() AS (CAST("BEN" AS STRING));
-- CREATE TEMP FUNCTION port_label() AS (CAST("LOME" AS STRING));


## save table if needed:
CREATE TABLE `world-fishing-827.scratch_joef.vessel_list_BEN_2021-23_pipe3` AS
...
--------------------------------------
  pull_by_anchorage AS(
    SELECT
      ...
      num_fishing
    FROM clean_info
    --- commented out for now as including all BEN anchorages

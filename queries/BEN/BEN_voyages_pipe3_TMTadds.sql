--------------------------------------
  -- Query to quantify foreign fishing and carrier vessel visits and unique vessels visiting Benin.
  --
...
CREATE TABLE `world-fishing-827.PortsProgramme.BEN_voyages_2021-23` AS
 ...
 additional_vessels AS(
   SELECT DISTINCT
    ...
    WHERE
     ...
     AND ssvid IN ("341032000", "412678562", "799180815", "799180816", "352002156") -- add additional vessels here
    ),
...
----------------------------------------------------------
  nn_bunkers_high_confidence AS (
    ...
    FROM
      `pipe_ais_v3_published.vi_ssvid_byyear_v20240901` --
    WHERE
      ...
      AND ssvid IN (
        SELECT ssvid
        FROM `pipe_ais_v3_published.identity_core_v20241001`
...
--------------------------------------
  ...
  # filter for ports of interest -- using anchorages list for Monrovia in this case
      OR ssvid IN ("341032000", "412678562", "799180815", "799180816", "352002156") -- adding in TMT missing vessels
...
-- pull_by_anchorage AS(
  ...
  LEFT JOIN (
    ...
    FROM `PortsProgramme.BEN_anchorages_TMTreviewed`)

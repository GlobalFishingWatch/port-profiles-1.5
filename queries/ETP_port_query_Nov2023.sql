--------------------------------------
  ...
  --    hours currently)
CREATE TEMP FUNCTION  start_date() AS (TIMESTAMP('2013-01-01 00:00:00 UTC'));
CREATE TEMP FUNCTION  start_year() AS (2013);
...
WITH
  ...
  FROM (
    ...
    INNER JOIN (
      SELECT
        ...
        last_timestamp
      FROM
        vessel_database.all_vessels_v20231001
  ...
  --------------------------------------
  -- ID duplicate voyages and assign same characteristics
  -- (either/both flag or vessel class are the apparent issues)
  -- ***note choosing first is arbitrary, need better rule
  --------------------------------------
duplicate_voyages AS(
  SELECT
  ssvid,
  year,
  vessel_id,
  ...
  FIRST_VALUE (vessel_iso3) OVER (
    PARTITION BY year, ssvid, vessel_id, trip_start, trip_end
    ORDER BY trip_end ASC, vessel_iso3) AS vessel_iso3,
  FIRST_VALUE (vessel_class) OVER (
    PARTITION BY year, ssvid, vessel_id, trip_start, trip_end
    ORDER BY trip_end ASC, vessel_class) AS vessel_class,
  FROM c4_voyages
),

collapsed_voyages AS(
  SELECT
   ssvid,
   year,
   vessel_id,
  ...
  SELECT
    ...
    iso3
  FROM
    proj_pew_ports.gfw_ports_database_v20230424),
...
-- current start and end fail - v2 query for entry port uses the version with both current
-- start and prev end, etc
---------------------------------------------------------------------------------
  block_start_end AS (
    SELECT
    *,

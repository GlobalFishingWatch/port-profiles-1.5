...
WITH
  --------------------------------------
  -- Get fishing vessel port visits - using vi_ssvid fishing_list_best
  --------------------------------------
 visits_fishing AS (
  SELECT
    *
  FROM (
    SELECT
      ...
      EXTRACT(year
      FROM
        end_timestamp) AS year,
      duration_hrs,
      confidence
    FROM
      pipe_ais_v3_alpha_published.port_visits
    WHERE
      end_timestamp >= start_date() AND start_timestamp <= end_date()
      AND confidence >= 2
)
  LEFT JOIN (
    SELECT
  ...
  --------------------------------------
  -- Get carrier visits using -- vessel database/all vessels
  --------------------------------------
visits_carrier AS (
  SELECT
    *
  FROM (
    ...
    ON visits.ssvid = vessels.ssvid
      AND visits.end_timestamp
  ...
  --------------------------------------
  -- Combine carrier and fishing visits
  --------------------------------------
  initial_visits AS (
  SELECT
    *
  ...
  FROM (
    ...
    LEFT JOIN
      anchorage_names b
    ON
      start_anchorage_id = s2id)
  LEFT JOIN
    anchorage_names c
  ON
    end_anchorage_id = s2id),

    --  select * from named_visits where start_label IN ("CONAKRY") OR end_label IN ("CONAKRY") /*

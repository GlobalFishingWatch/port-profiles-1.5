...
CREATE TABLE `world-fishing-827.PortsProgramme.TGO_masterlist_2021-23` AS
...
voyages AS (
  ...
  FROM `world-fishing-827.PortsProgramme.TGO_voyages_2021-23` AS a
  LEFT JOIN (
    SELECT
      trip_id,
      -- total_voyage_h,
...
add_anchorages AS(
  ...
  FROM (
    ...
    -- a.trip_end_anchorage_id,
        port_or_anchorage,
        GhanaEEZ,
      FROM `world-fishing-827.PortsProgramme.TGO_anchorages_TMTreviewed`) as b
      -- USING (s2id) )
...
events AS (
  SELECT
    ...
    CAST(NULL AS string) AS port_or_anchorage,
    CAST(NULL AS string) AS GhanaEEZ,
    CAST(NULL AS numeric) AS port_distance_from_shore_m,
    CAST(NULL AS BOOL) AS at_dock,
    CAST(NULL AS int64) AS port_confidence
  FROM `world-fishing-827.PortsProgramme.TGO_events_2021-23` AS a
  LEFT JOIN (
    SELECT
      trip_id,
      vessel_id,
